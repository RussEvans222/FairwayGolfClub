...
export function SessionWorkspaceScreen({
  ...
  return (
    <div className="w-full h-full overflow-hidden px-6 py-6 lg:px-10 lg:py-8">
      <div className="grid h-full gap-6 lg:grid-cols-[minmax(0,1.1fr)_minmax(360px,0.9fr)]">
        <section className="flex min-h-0 flex-col gap-5 rounded-[2rem] border border-[#2A2A2A] bg-[#0E0E0E] p-6 lg:p-8">
          <div className="flex items-start justify-between gap-4 flex-shrink-0">
            <div>
              <img src="/images/logo-text.png" alt="Fairway Golf Club" className="h-10 w-auto bay-logo-white mb-3" />
          ...
          <div className="grid gap-5 xl:grid-cols-[1.15fr_0.85fr] min-h-0 flex-1">
            <div className="flex min-h-0 flex-col gap-5 rounded-3xl border border-[#2A2A2A] bg-[#111] p-5">
            ...
            <div className="grid gap-5 min-h-0">
              ...
              <div className="flex min-h-0 flex-col rounded-3xl border border-[#2A2A2A] bg-[#111] p-5">
                <div>
        ...
        <aside className="flex min-h-0 flex-col gap-5 rounded-[2rem] border border-[#2A2A2A] bg-[#0B0B0B] p-6 lg:p-8">
          <div className="rounded-3xl border border-[#2A2A2A] bg-[#111] p-5">
            <p className="text-[#C9A84C] text-xs uppercase tracking-[0.3em]">Session state</p>
            <h3 className="text-white text-2xl font-bold mt-1">{started ? 'Session started' : 'Ready to start'}</h3>
            <p className="text-[#666] text-sm mt-2">
              {started
                ? 'The workspace is open and ready to receive live telemetry from the local relay.'
                : 'Tap Start session to open the full workspace before the first swing.'}
            </p>

            <div className="mt-4 grid gap-2 text-sm">
              <SideStat label="Bay" value={bay.name} />
              <SideStat label="Telemetry" value={telemetry.connectionState} />
              <SideStat label="Players" value={String(players.length || 0)} />
              <SideStat label="Last update" value={telemetry.lastUpdatedAt ? formatTime(telemetry.lastUpdatedAt) : 'Waiting'} />
            </div>
          </div>

          <div className="rounded-3xl border border-[#2A2A2A] bg-[#111] p-5">
            <p className="text-[#C9A84C] text-xs uppercase tracking-[0.3em]">Player context</p>
            <h3 className="text-white text-xl font-bold mt-1">{player?.displayName ?? 'No player selected'}</h3>
            <p className="text-[#666] text-sm mt-2">
              {player?.lifetimeSummary?.currentFocus ?? 'Performance context will appear as soon as a tracked golfer is active.'}
            </p>
            {players.length > 1 && (
...
function SideStat({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-4 rounded-2xl border border-[#222] bg-[#0D0D0D] px-4 py-3">
      <span className="text-[#666] text-xs uppercase tracking-[0.3em]">{label}</span>
