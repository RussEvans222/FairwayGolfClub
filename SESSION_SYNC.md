...
**Last updated:** 2026-07-11 (kiosk visual redesign session, see "Session Update — 2026-07-11 (Heritage Green kiosk redesign)" immediately below)  
**Last commit:** pushed to `main` this session (RussEvans222/FairwayGolfClub) — see note below for the preceding commit this built on

**For whoever picks this up next:** the kiosk's Welcome screen and check-in selection ("Who's Playing?" / `PlayerTypeScreen`) were re-skinned to a new "Heritage Green" design system (Playfair Display + Work Sans fonts, forest-green/cognac/gold palette, full-bleed real interior photography, glass-gold buttons) per Stitch mockups in `Assets/stitch_fairway_sim_kiosk_redesign/`. Scope was deliberately limited to those two screens plus shared tokens — **all other kiosk screens (PIN entry, guest registration, payment, session summary, bay queue, staff login body copy, etc.) still use the old flat dark/gold look**, they were not touched. The real "Fairway Golf Club" wordmark (`logo-text.png`, white-filtered via CSS) now replaces the placeholder circle-F mark / text-only brand label everywhere it appeared in the kiosk (Welcome, check-in selection, staff login, scheduled-sessions header) and on the Salesforce-side bay greeting screen (`fairwayBayGreeting` LWC) — that LWC change **is not yet deployed to the org**, only committed to the repo. See prior entries below for the still-outstanding Salesforce backend/kiosk items (Checked In status ladder live-test, PaymentLink checkout error, etc.) — none of that was touched this session.
...
7. **Guest "join a session" flow (kiosk) — SUPERSEDED 2026-07-10, see `BOOKING_CHECKIN_PROCESS.md` Case 2**
   - Original design below was a *silent* join (no approval step) — Russell's fuller spec in `BOOKING_CHECKIN_PROCESS.md` replaces this with an **approval-gated** version: requester presses the active session, bay screen pops "[name] is attempting to join — Approve/Deny," host approves or denies. Treat that doc as current design; this entry kept for history.
   - Original (now superseded) design: New Guest → "Join a Session" → see active bays with member names → select → free `Golfer_Profile__c` + `Session_Participant__c` created → directed to bay, bay display silently adds them as a new player tab
   - **Not yet built**
